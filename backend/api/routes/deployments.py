...
async def get_stats(db: AsyncSession = Depends(get_db)):
    """
    Returns aggregate counts for the overview cards.
    Combines DB deployment stats + live Kubernetes data.
    """
...
async def trigger_deployment(
    ...
    """
    Accepts repo URL, creates DB record, triggers Jenkins in the background.
    Returns immediately with deployment_id — frontend polls /deployments/{id}.
    """
...
async def list_deployments(
    ...
    return DeploymentListOut(
        total=total,
        items=[_enrich(d) for d in items],
    )
...
async def get_deployment_status(
    ...
    """Lightweight endpoint for frontend polling — returns only status fields."""
    dep = await svc.get_deployment(db, deployment_id)
    if not dep:
...
# ── WebSocket — real-time deployment status ───────────────────────────────────
...
async def deployment_websocket(
    ...
    """
    WebSocket endpoint — streams status updates for a single deployment.
    Frontend connects once and gets live updates without polling.

    Protocol:
      Server → Client: JSON { status, jenkins_build_number, duration_seconds, finished_at }
      Connection closes automatically when deployment reaches terminal state.
    """
...
def _enrich(dep) -> DeploymentOut:
    """Convert ORM model to Pydantic schema, adding computed fields."""
